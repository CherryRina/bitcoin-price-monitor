...
def get_and_save_bitcoin_price(logger, json_loader):
    ...
    scheduler.enter(60, 1, get_and_save_bitcoin_price, kwargs={"logger": logger, "json_loader": json_loader})


def generate_graph(logger, json_loader):
    graph_generator = BitcoinGraphGenerator(json_loader=json_loader, logger=logger)
    graph_generator.generate_graph()
    scheduler.enter(3600, 1, generate_graph, kwargs={"logger": logger, "json_loader": json_loader})
...
def main():
    ...
    scheduler.enter(3600, 1, generate_graph, kwargs={"logger": logger, "json_loader": json_loader})
    scheduler.enter(3600, 2, send_email, kwargs={"logger": logger, "json_loader": json_loader})
    scheduler.run()
